          ...
          <template v-if="(!item.hide || item.hide.indexOf('update')<0)">
            ...
            <template v-if="item.input">
              <template v-if="(item.input.type == 'datetime')">
                <el-date-picker v-show="isUpdate" @input="$forceUpdate()" v-model="updateObj[item.prop]" type="datetime" :value-format="item.input.format || 'timestamp'" placeholder="选择日期时间"></el-date-picker>
              </template>
