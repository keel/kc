...
const prop = {
  ...
  'fields': [ //
    ...
    { 'col': 'url', 'name': '视频url', 'type': 'string' },
...
upload.addUploadAction('uploadVideo', function(file, fields, resp) {

  // vlog.log('uploadActions:datafilter: file:%j, fields:%j', file, fields);

  vlog.log('上传视频成功:', file.filepath);
...
exports.router = function() {
  ...
  ci.router.get('*', function(req, resp, next) { // eslint-disable-line
    ...
    resp.send(render.list({ 'tb': prop.tb, 'tbName': prop.tbName, 'hideNew': !userPermission[prop.tb + '/add'], 'isVideos': true }));
  });
  return ci.router;
};
