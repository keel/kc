...
const login = function(req, resp, callback) {
  ...
  let userStr;
  try {
    pwdStr = Buffer.from(reqData.p, 'base64').toString();
    userStr = Buffer.from(reqData.u, 'base64').toString();
    // pwdStr = pwdStr.substring(pwdStr.indexOf('@') + 1);
    // console.log('=====user:[%s]', userStr);
    // console.log('=====pwd:[%s]', pwdStr);
  } catch (e) {
    return callback(null, error.json('auth', '用户名密码验证失败，请重试.'));
  }
  ...
  if (firstUser && firstUser.isFirst) {
    if (userStr === firstUser.loginName && pwdStr === firstUser.loginPwd) {
  ...
  fail2ban.checkBan(userStr, (err, waitHours) => {
    if (err) {
