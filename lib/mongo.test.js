...
require('./mongo').init(configFile).c('user').insert({ 'phone': '13311112223', 'createTime': new Date().getTime() });
...
mongo.c('user').query({ 'phone': '13311112222' }, { limit: 2 }, (e, re) => {
  if (e) {
    return vlog.eo(e);
  }
  console.log(re);
...
const test = () => {
  ...
  mongo2.init(configFile, (e) => {
    ...
    mongo.c('user').find({ 'phone': '13311112223' }, (e, cur) => {
      if (e) {
        return vlog.eo(e);
      }
      //这里需要toArray
