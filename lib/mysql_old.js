...
var initCallbacks = [];
var isInitStart = false;

var callbackInits = function(err, re) {
  for (var i = 0, len = initCallbacks.length; i < len; i++) {
    initCallbacks[i](err, re);
  }
  isInitStart = false;
};
...
var init = function(configFile, callback) {
  ...
  if (isCluster) {
    ...
    pool.getConnection(function(err, connection) {
      if (err) {
        return callbackInits(vlog.ee(err, 'mysql cluster init getConnection'));
      }
      connection.release();
      vlog.log('mysql cluster init OK. %j', hosts);
      callbackInits(null, 'ok');
    });
    return;
  }
  //非集群
  ...
  pool.getConnection(function(err, connection) {
    if (err) {
      return callbackInits(vlog.ee(err, 'mysql init getConnection'));
    }
    connection.release();
 ...
 * @param  {function} reObjFn   根据存储过程结果产生API输出
  ...
  */
var createProcApi = function(procName, apiKeyIn, parasInFn, reObjFn, beforeProc) {
  ...
  var procDo = function(req, resp, callback) {
    ...
    if (beforeProc) {
      beforeProc(req, reqDataArr[1], function(err, re) {
        ...
        //beforeProc可以中断执行
        if (re[0] !== 0) {
          var respObj = iApi.makeApiResp(re[0], re[1], apiKeyIn);
          //返回
