...
var init = function(configFile, callback) {
  ...
  initDB(kconfig.getConfig().mongoDbName, kconfig.getConfig().mongoUrl, function(err) {
    if (err) {
      if (callback) {
        callback(vlog.ee(err,'db:init',configFile));
      }else{
        vlog.eo(err, 'init', configFile);
      }
      return;
    }
    redis.initRedis(function(err) {
      if (err) {
        if (callback) {
          return callback(vlog.ee(err, ''));
        }else{
          vlog.eo(err,'db:initRedis',configFile);
        }
        return;
      }
      if (callback) {
...
var checkColl = function(name, callback) {
  ...
  if (!cl) {
    if (!db) {
      initDB(kconfig.getConfig().dbName, kconfig.getConfig().mongoUrl, function(err) {
        if (err) {
