...
// });
const checkIndex = function(tableName, newIndexes, isFirst = false, configName = 'default') {
  if (isFirst) {
    // vlog.log('first create indexes');
    for (const i in newIndexes) {
      const targetIndex = newIndexes[i];
      checkColl(tableName).createIndex(targetIndex);
      vlog.log('[%s] index created: %j', tableName, targetIndex);
    }
    return;
  }
  checkColl(tableName, configName).indexes((err, indexRe) => {
    if (err) {
      vlog.eo(err, 'checkIndex');
      //一般是未创建表,这里直接使用首次创建模式
      checkIndex(tableName, newIndexes, true);
      return;
    }
 ...
 */
const pCheckColl = (tableName, configName = 'default') => {
  return new Proxy({}, {
    get(target, action) {
      // console.log('action:%s', action, tableName, configName);
      return (...paras) => {
...
const getDb = function getDb(configName = 'default', callback) {
  ...
  return getHelper(configName).getClient(callback);
};
