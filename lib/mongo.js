...
const logToDb = (coll) => (logObj, callback) => {
  if (!callback) {
    callback = (e) => {
      if (e) {
        vlog.eo(e, 'logToDb', logObj);
      }
    };
  }
...
const getDb = (callback) => {
  ...
  helper.init(false, (err, database) => {
    if (err) {
      callback(vlog.ee(err, 'getDb:helper.init'));
      return;
    }
