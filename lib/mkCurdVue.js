...
const vueCurdPath = Path.join(__dirname, '../vue/src/views/curd');
const tplPath = Path.join(__dirname, '../templets/curd.vue');


//这里定义需要处理的标签
const replaceTagArr = ['tb', 'tbName'];
...
const make = function(confObj) {
  const destFile = Path.join(vueCurdPath, confObj.tb + '.vue');
