...
var createApp = function(dir, options) {
  ...
  if (config.redis && config.redis.init) {
    redis.init(configFile,function (err) {
      if (err) {
        return vlog.eo(err);
      }
      app.emit('redisInitOK');
    });
  }
  if (config.mongo && config.mongo.init) {
    mongo.init(configFile,function (err) {
      if (err) {
        return vlog.eo(err);
      }
      app.emit('mongoInitOK');
    });
  }
  if ((config.mysql && config.mysql.init) || (config.mysqlCluster && config.mysqlCluster.init)) {
    mysql.init(configFile,function (err) {
      if (err) {
        return vlog.eo(err);
      }
      app.emit('mysqlInitOK');
    });
  }
