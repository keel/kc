...
const createApp = function(dir, options) {
  let configFile;
  if (!options || !options.configFile) {
    configFile = dir + '/config.json';
  }
  const configFileStats = fs.statSync(configFile);
  ...
  const config = ktool.kconfig.init(configFile);
  ...
  for (let i = 0,len = initsArr.length; i < len; i++) {
    ...
    dbObj.init(configFile, function(err) {
      if (err) {
        vlog.eo(err, '');
        return;
      }
