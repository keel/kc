...
const sessionGet = function(req, resp, key, callback) {
  ...
  sessionStore.get(sessionPre + sessionId, (err, value) => {
    if (err) {
...
const setAuthed = function(req, resp, userId, userLevel, callback, moreParas) {
  const sessionValue = {
    userId,
    userLevel,
    'startTime': Date.now(),
  ...
  if (moreParas) {
    for (const i in moreParas) {
      sessionValue[i] = moreParas[i];
    }
  }
...
const updateAuthed = function(req, resp, sid, sessionValue, callback) {
  ...
  authSessionSet(resp, sid, JSON.stringify(sessionValue), expireTime, callback);
};
...
const sessionCheck = function(sid, req, resp, callback) {
  ...
  sessionStore.get(sessionPre + sid, function(err, sessionValue) {
    if (err) {
