...
const dbTypeMap = {
  'mongo': {
    'fetchData': (tableName, query, options, callback) => {
      ...
      let dbConfigName = 'default';
      if (options.dbConfigName) {
        dbConfigName = options.dbConfigName;
        delete options.dbConfigName;
      }
      dbMap.mongo.c(tableName, dbConfigName).query(query, options, callback);
    },
    'dbName': 'mongo',
    'isInit': false
  },
...
const cacheByKey = function(cacheObj, keyArr, docs, tableName, callback) {
  ...
  for (let i = 0; i < sum; i++) {
    const doc = docs[i];
    // for (const j in doc) {
    //   if (j !== '_id' && (cck.check(doc[j], 'json') || cck.check(doc[j], 'array'))) {
    //     doc[j] = JSON.stringify(doc[j]);
    //   } else {
    //     doc[j] = doc[j] + '';
    //   }
    // }
...
const cacheMakeWithConf = function(cacheType, dbType, tableParasArr, configName, callback) {
  ...
  if (configName) {
    options.dbConfigName = configName;
  }
  ...
  cacheTable(cacheType, dbType, tableName, key, query, options, function(err) {
    ...
    cacheMakeWithConf(cacheType, dbType, tableParasArr, configName, callback);
  });
};

const cacheMake = function(cacheType, dbType, tableParasArr, callback) {
  cacheMakeWithConf(cacheType, dbType, tableParasArr, null, callback);
};
...
const set = function(cacheType, key, val, callback = ktool.defaultCallback(null, __filename)) {
  const cacheObj = cacheTypeMap[cacheType];
  if (!cacheObj) {
    return callback(vlog.ee(new Error('cacheType or dbType error'), cacheType, key));
  }
  return cacheTypeMap[cacheType].set(key, val, callback);
};
...
exports.get = get;
exports.set = set;
exports.cacheMakeWithConf = cacheMakeWithConf;
