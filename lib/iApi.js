...
const initAct = function(actName, iiObj, router, iiConfig) {
  ...
  router.post('/' + actName, preCheck, function(req, resp, next) {
    ...
    if (iiObj.authPath) {
      if (!req.sessionValue || !req.sessionValue.userPermission || !Auth.auth(iiObj.authPath, req.sessionValue.userPermission)) {
        return resp.status(validatorFailStateCode).send(error.json('auth'));
      }
    }
...
const getRouter = function(iiConfig) {
  ...
  for (const act in actions) {
    ...
    if (iiConfig.authPath) {
      const authPath = iiConfig.authPath + '/' + act;
