...
var callbackInits = function(err, re1, re2) {
  for (var i = 0, len = initCallbacks.length; i < len; i++) {
    initCallbacks[i](err, re1, re2);
  }
  isInitStart = false;
};
...
var init = function(configFile, callback) {
  ...
  if (redisClient) {
    isInitStart = false;
    return callback(null, kconfig.getConfig().redis.redisIP, kconfig.getConfig().redis.redisPort);
  }
  if (callback) {
    initCallbacks.push(callback);
  }
  ...
  initRedis(function(err, re1, re2) {
    if (err) {
      return callbackInits(err);
    }
    callbackInits(null, re1, re2);
  });
};
...
var initRedis = function(callback) {
  ...
  redisClient.on('error', function(err) {
    ...
    redisClient = null;
    callback(vlog.ee(err, 'redis init'));
    return;
  });
...
var hmset = function(cacheKey, value, callback) {
  checkClient(function(err, client) {
    if (err) {
      return callback(err);
    }
