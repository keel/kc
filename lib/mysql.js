...
const dbGetConn = function dbGetConn(clusterNode = defautClusterNode, clusterSelector = defautClusterSelector, callback) {
  if (!kconfig.getConfig().mysql.isCluster) {
...
const initClient = (callback) => {
  const configObj = kconfig.getConfig().mysql;
  const isCluster = configObj.isCluster;
  if (!isCluster) {
    pool = mysql.createPool(configObj.server);
  ...
  for (const i in configObj.servers) {
    hosts.push(i + ':' + (configObj.servers)[i].host + ' db:' + (configObj.servers)[i].database);
    pool.add(i, (configObj.servers)[i]);
  }
